...
- A Jupyter environment (Jupyter Notebook or Google Colab) to run the `.ipynb` file.
...
> Note: Because the 100 labeled training points are chosen randomly from the training set, the exact accuracies and numbers of pseudo-labeled points can vary slightly between runs. The overall behaviour of each confidence threshold, however, remains the same.
...
- Interpret how pseudo-labels from unlabeled data influence accuracy.
